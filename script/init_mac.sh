...
# 覆盖本地配置
curl https://raw.githubusercontent.com/csh1314/workflow/main/config/.zshrc >~/.zshrc
curl https://raw.githubusercontent.com/csh1314/workflow/main/config/.gitconfig >~/.gitconfig

# 注意用户名确保和邮箱名相同
git config --global user.name csh1314
git config --global user.email cshsb@qq.com
...
# jq 命令行处理 json 工具
# itermocil 保存终端基础配置
brew install python nvm iterm2 visual-studio-code
